...
function NewPurchaseOrderModal({ onClose, onCreated }) {
  ...
  useEffect(() => {
    ...
    api.get('/products', { params: { page_size: 200 } }).then(({ data }) => setProducts(data.data));
  }, []);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={addItem} className="text-xs font-semibold text-petrol-700 hover:underline">+ Agregar línea</button>
            </div>
            {items.map((it, idx) => (
