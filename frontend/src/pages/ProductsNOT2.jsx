...
const emptyForm = {
  ...
  tax_rate_type: 'general', stock_quantity: '', min_stock: '5', unit: 'unidad', supplier_ids: [],
};
...
export default function Products() {
  ...
  useEffect(() => { api.get('/suppliers', { params: { page_size: 100 } }).then(({ data }) => setSuppliers(data.data)); }, []);
  ...
  const openEdit = async (p) => {
    ...
    setForm({
      ...
      stock_quantity: p.stock_quantity, min_stock: p.min_stock, unit: p.unit, supplier_ids: [],
    });
    setError('');
    setShowForm(true);
    // El listado no trae supplier_ids; se pide el detalle completo para saber
    // qué proveedores ya tiene asignados este producto.
    try {
      const { data } = await api.get(`/products/${p.id}`);
      setForm((f) => ({ ...f, supplier_ids: data.supplier_ids || [] }));
    } catch {
      // si falla, el formulario queda usable igual, solo sin preseleccionar proveedores
    }
  };
      ...
      {showForm && (
        <Modal onClose={() => setShowForm(false)} title={editing ? 'Editar producto' : 'Nuevo producto'}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="label-field">Proveedores que suministran este producto</label>
              {suppliers.length === 0 ? (
                <p className="text-sm text-petrol-900/40">No hay proveedores registrados todavía.</p>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-1.5 max-h-40 overflow-y-auto border border-petrol-900/10 rounded-lg p-3">
                  {suppliers.map((s) => (
                    <label key={s.id} className="flex items-center gap-2 text-sm text-petrol-900/80">
                      <input
                        type="checkbox"
                        ...
                        onChange={(e) => {
                          setForm((f) => ({
                            ...
                            supplier_ids: e.target.checked
                              ? [...f.supplier_ids, s.id]
                              : f.supplier_ids.filter((id) => id !== s.id),
                          }));
                        }}
                      />
                      {s.name}
              ...
              <p className="text-xs text-petrol-900/40 mt-1">
                Esto determina qué productos aparecen al elegir este proveedor en Compras.
              </p>
            </div>
