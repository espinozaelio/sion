...
// Genera un enlace de restablecimiento para un usuario, a pedido de un administrador
// YA AUTENTICADO (esta ruta vive bajo requireRole('admin') en userRoutes.js). A diferencia
// de /api/auth/forgot-password, aquí no hay riesgo de que un desconocido tome una cuenta
// ajena: solo alguien que ya tiene acceso de administrador puede generar el enlace,
// y queda registrado quién lo generó y para quién.
async function generateResetLink(req, res, next) {
  try {
    const { id } = req.params;
    const { rows } = await pool.query('SELECT id_usuario, correo, activo FROM usuarios WHERE id_usuario = $1', [id]);
    if (!rows[0]) return res.status(404).json({ message: 'Usuario no encontrado.' });
    if (!rows[0].activo) return res.status(400).json({ message: 'El usuario está desactivado.' });

    await pool.query(
      'DELETE FROM restablecimientos_contrasena WHERE id_usuario = $1 AND usado = false',
      [id]
    );
    ...
    await pool.query(
      `INSERT INTO restablecimientos_contrasena (id_usuario, token, expira_en) VALUES ($1, $2, $3)`,
      [id, token, expiresAt]
    );

    res.json({ reset_token: token, expires_at: expiresAt, correo: rows[0].correo });
  } catch (err) {
    next(err);
  }
}

module.exports = { list, getOne, create, update, remove, generateResetLink };
