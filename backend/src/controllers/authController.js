...
async function forgotPassword(req, res, next) {
  try {
    ...
    // Respuesta genérica idéntica exista o no el correo, y SIN incluir el token:
    // devolver el token aquí permitiría a cualquiera tomar el control de una cuenta
    // ajena con solo conocer su correo, sin necesitar acceso real a ese correo.
    const generic = { message: 'Si el correo existe, se generó un enlace de restablecimiento. Revisa tu correo, o pide a tu administrador que te genere el enlace desde el módulo de Usuarios.' };
    if (!rows[0]) return res.json(generic);

    // Invalida enlaces anteriores sin usar de este usuario antes de crear uno nuevo.
    await pool.query(
    ...
    // TODO producción: enviar `token` por correo real (ej. Resend, SendGrid, Gmail SMTP)
    // en vez de solo registrarlo. Mientras no haya correo configurado, el enlace
    // solo se puede obtener de forma segura vía un administrador ya autenticado
    // (ver POST /api/users/:id/reset-link).
    res.json(generic);
  } catch (err) {
    next(err);
  }
